    ...
    this.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        void RE_Direct_Scattering(const in IncidentLight directLight, const in vec2 uv, const in vec3 geometryPosition, const in vec3 geometryNormal, const in vec3 geometryViewDir, const in vec3 geometryClearcoatNormal, inout ReflectedLight reflectedLight) {
          ...
          #else
            // diffuseColor.rgb — актуально для новых версий three
      ...
      // --- ЗАМЕНА replaceAll на split().join() ---
      const needle = 'RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );';
      const injected = `
        RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
        RE_Direct_Scattering(directLight, vUv, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, reflectedLight);
      `;
      const lightsChunk = ShaderChunk.lights_fragment_begin.split(needle).join(injected);
...
// утилита: считаем, что это интерактивный элемент
const isInteractive = (el: Element | null) =>
  !!(el && el.closest('button, a, [role="button"], [role="link"], input, textarea, select'));

function onTouchStart(e: TouchEvent) {
  if (e.touches.length > 0) {
    pointerPosition.set(e.touches[0].clientX, e.touches[0].clientY);

    // ни при каких условиях НЕ мешаем тапу на кнопке/ссылке
    const startedOnInteractive = isInteractive(e.target as Element);
    for (const [elem, data] of pointerMap) {
      const rect = elem.getBoundingClientRect();
      if (isInside(rect)) {
        data.touching = true;
    ...
    // ВАЖНО: на touchstart больше НИЧЕГО не prevent-им,
    // чтобы клик по кнопке дошёл до onClick.
  }
}
  ...
  if (e.touches.length > 0) {
    ...
    for (const [elem, data] of pointerMap) {
      ...
      if (inside) {
        ...
        // жест начался на канвасе и продолжается вне — не даём странице скроллить
        shouldPrevent = true;
        data.onMove(data);
      }
    }

    // Блокируем скролл/зум только когда реально двигаем фон,
    // и только если жест НЕ начался на кнопке/ссылке.
    if (shouldPrevent && !startedOnInteractive) {
      e.preventDefault();
    }
  }
}
