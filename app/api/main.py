...
from app.observability.metrics import PromHTTPMiddleware, metrics_app  # metrics_app не монтируем на "/"
...
async def _search_pg_fallback(
    ...
    # Собираем SQL динамически, чтобы не городить лишних условий.
    conds: List[str] = []
    args: List[Any] = []
    i = 1
...
async def lifespan(app: FastAPI):
    ...
    async def _init_conn(con: asyncpg.Connection):
        # Возвращаем json/jsonb как Python-объекты
    ...
    # Telegram-логирование WARN/ERROR
    ...
    if TELEGRAM_LOG_BOT_TOKEN and TELEGRAM_LOG_CHAT_ID:
        ...
        # пусть uvicorn/httpx/telethon пробрасывают в корень
        for name in ("uvicorn.error", "uvicorn.access", "httpx", "telethon"):
            logging.getLogger(name).propagate = True
        await tg_handler.start()

        # --- Менеджер тем и event-лог по действиям пользователя ---
    ...
    try:
        # 👇 запускаем фоновые лайв-мониторы (инфра-дашборд + при желании — агрегаты приложения)
        await start_live_monitors(app)
        # 🔔 лог-shipper: каждую ошибку отдельным сообщением
...
# --- OGMA telemetry middleware ---
@app.middleware("http")
async def _ogma_metrics_mw(request: Request, call_next):
    user = request.headers.get("X-User-Id", "anon")
    # считаем визит на каждый запрос к API
    try:
        mark_visit("api", user)
    except Exception:
        pass
    ...
    try:
        ...
        # необработанная ошибка → метрика + лог
        try:
            mark_error(request.url.path, 500)
        except Exception:
            pass
        raise
    finally:
        # тут можно было бы снимать latency, но его уже даёт Instrumentator
        _ = time.perf_counter() - start

    # считаем ошибки 4xx/5xx
    try:
        if response.status_code >= 400:
            mark_error(request.url.path, response.status_code)
    except Exception:
        pass
    return response

# НЕ монтируем metrics_app на "/" (ломает все маршруты).
# Если нужно отдельное приложение с метриками — оставляем только /metrics через Instrumentator.
# app.mount("/metrics", metrics_app)  # ← не нужно, конфликтует с Instrumentator
app.add_middleware(PromHTTPMiddleware)

# ── CORS: без '*' при credentials; список из ENV иначе dev-дефолт ─────────────
_allow_origins = [
...
app.add_middleware(
    ...
    allow_credentials=True,  # у вас fetch с credentials: "include"
    allow_methods=["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"],
    allow_headers=[
        "*"
    ],  # ваши кастомные заголовки (X-Telegram-Init-Data, Range, If-Range и т.д.)
    # чтобы фронт видел ключевые заголовки стрима (полезно для плееров и дебага)
    expose_headers=[
        "Accept-Ranges",
        "Content-Range",
        "Content-Length",
...
# Ловим необработанные ошибки и отправляем в логи (метрики уже считает middleware выше)
@app.middleware("http")
...
# Экспозиция метрик Prometheus (на 127.0.0.1:8080/metrics)
...
# (search_router второй раз не добавляем — был дубликат)

logger = logging.getLogger("app")

# ---- helper: user id из Telegram WebApp (если есть подпись) ----
try:
...
async def telemetry_visit(request: Request, source: str = Query(default="webapp")):
    ...
    mark_visit(source=source, user=user)
    ev = getattr(app.state, "eventlog", None)
    if ev:
        ...
        await ev.send(
            "visits",
            f"👋 <b>visit</b> user=<code>{user}</code>\nUA: <code>{ua}</code>\nRef: <code>{ref}</code>",
        )
    return {"ok": True}
...
@app.get("/api/_boom")
def _boom():
    # Специально кидаем необработанную ошибку → попадёт в TelegramHandler
...
async def search_tracks(
    ...
    if ev:
        ...
        await ev.send(
            "search",
            f"🔎 <b>search</b> user=<code>{user}</code>\nq=<code>{q}</code>\n{ftxt}",
        )

    # --- сначала пробуем Meili ---
    try:
        ...
        # 401/403/5xx/сетевые — уходим в PG без падения ручки
        return await _search_pg_fallback(q, limit, offset, artist, hashtag, chat, sort)


# Прямой экспорт REGISTRY (если нужен)
@app.get("/_metrics")
def _metrics():
