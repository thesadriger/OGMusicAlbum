...
const SplitText: React.FC<SplitTextProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const assignTargets = (self: GSAPSplitText) => {
        ...
        if (type.includes("chars") && self.chars?.length) targets = self.chars;
        if (!targets.length && type.includes("words") && self.words?.length)
          targets = self.words;
        if (!targets.length && type.includes("lines") && self.lines?.length)
          targets = self.lines;
  ...
  const renderTag = () => {
    ...
    const classes = `split-parent overflow-hidden inline-block ${className}`.trim();
