...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    this.renderer = new THREE.WebGLRenderer({
      antialias: false,
      alpha: false, // канвас непрозрачный, фон не «пропадает»
  ...
  initPasses() {
    if (this.composer) return; // уже инициализировано

    const gl = this.renderer.getContext() as any;
    const attrs = gl?.getContextAttributes?.();
    if (!gl || !attrs || (gl as any).isContextLost?.()) {
      console.warn('[Hyperspeed] WebGL context is lost, skip init');
      this.disposed = true;
      return;
    }
  ...
  init() {
    if (this.disposed) return; // если успели разобрать инстанс - выходим тихо
    this.initPasses();
    if (this.disposed) return; // initPasses мог поставить disposed при потерянном контексте
    const options = this.options;
    this.road.init();
  ...
  dispose() {
    ...
    // корректно теряем контекст WebGL (важно!)
    ...
    // удалим сам canvas из DOM, чтобы точно не висел
    try { this.renderer.domElement?.parentElement?.removeChild(this.renderer.domElement); } catch { }
  }
...
const Hyperspeed: FC<HyperspeedProps> = (props) => {
  ...
  useEffect(() => {
    // убираем предыдущий инстанс, если был
    if (appRef.current) {
    ...
    // ассеты подгружаем параллельно; если отвалятся — сцена всё равно работает
    myApp.loadAssets().catch(() => { }).finally(() => {
      // ничего: SMAA уже внутри сам доберёт данные с dataURL
    });

    // cleanup при размонтировании/смене опций
    return () => {
      if (appRef.current) {
        appRef.current.dispose();
        appRef.current = null;
